<?php
$EMAIL = 'vsenetak88@mail.ru'; //  Ваш e-mail на который будут приходить заявки
$FROM = 'vsenetak88@mail.ru';  //  e-mail отправителя (иногда требуется указать разрешенный настройками e-mail)
/*$REPLY = 'vsenetak88@mail.ru'; */  //  e-mail для ответа 
...
switch (@$_POST['action']) { // рассматриваем варианты разных значений поля action
    ...
    break;
    case 'service_order':
         sendmail("Запрос по вин-коду", render(
                        '<div><b>Vin-код:</b> '.stripinput($_POST['vin']).'</div>'.
                        '<div><b>Наименование детали:</b> '.stripinput($_POST['part']).'</div>'.
                        '<div><b>E-mail:</b> '.stripinput($_POST['email']).'</div>'.
                        '<div><b>Телефон:</b> '.stripinput($_POST['phone']).'</div>'.
                        '<div><b>Комментарий:</b> '.stripinput($_POST['intro']).'</div>'
        ),$EMAIL,$FROM,$REPLY);
        exit('200');
    break;
    default:
